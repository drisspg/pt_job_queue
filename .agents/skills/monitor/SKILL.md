...
`ptq supervise` fetches the latest Dr. CI comment, runs `~/dotfiles/scripts/github_ci_triage PR_URL`, saves transcripts under `agent_space/supervisor/JOB_ID/`, and classifies rows as `needs fix`, `unrelated CI`, `merge-ignore candidate`, or `needs human review`. A `merge-ignore candidate` should only lead to `@pytorchbot merge -i` when the PR is actually landing or a landing attempt just stopped. Use `uv run ptq supervise --no-triage` for a quick/offline render that does not call the triage helper.

For each row:

- `landing`: the PR is actively being merged; keep it under waiting/monitoring even if the CI column is red.
- `unrelated CI`: a landing attempt stopped on red checks that Dr. CI/HUD indicates are unrelated, flaky, or broken trunk. Suggest `gh pr comment PR_URL --body '@pytorchbot merge -i'`, but do not post it without user approval.
- `needs CI review`: an open PR has red CI whose relatedness is not settled by the raw monitor. Run `uv run ptq supervise --prompts` before proposing a code fix or human escalation.
- `needs human review`: a non-CI blocker needs human judgment, such as draft state, missing approval, rebase ambiguity, or an inconclusive deeper diagnosis.
...
3. If a landing attempt stopped and Dr. CI/HUD says the failures are unrelated, flaky, or broken trunk, report `unrelated CI` and propose `gh pr comment PR_URL --body '@pytorchbot merge -i'`.
4. If the PR is not currently landing, report unrelated/flaky red CI as `unrelated CI` without proposing merge-ignore. For approved PRs, say it is ready for a human merge-readiness decision; for unapproved or draft PRs, keep the non-CI blocker separate.
5. Use Dr. CI/HUD AI verdict text when present; badge images alone are not enough. Treat explicit AI text like “already existed at the merge base”, “same job, same error”, “unrelated to this patch”, or “no behavioral change” as evidence that the failing job is unrelated.
6. Only keep `needs fix` when the failure appears related to the PR's changed subsystem or the triage evidence shows a real regression.

Do not post `@pytorchbot merge -i`, rerun CI, push changes, or open a fixer workspace without user approval.

## Failing CI triage

Raw failing CI does not mean the PR caused the failure. Do not assume relation by chance; classify each failure from evidence before recommending any action.

Common false-positive pattern: an `adhoc` row such as `20260622-pytorch-adhoc-73da6c / adhoc / open / fail N` can be an issue-derived PR whose monitor row lost the issue context. For these, start from `uv run ptq peek JOB_ID` and the worklog/report, not the phase name. If triage failures are broad CI, runner, package, unrelated benchmark, or unrelated platform failures that do not touch the changed subsystem, place the row under waiting/human-judgment/skip rather than needs-action.

For `needs CI review` rows, prefer a supervisor sweep first:
...
Open raw logs only if the summary does not show the root error. If supervisor remains inconclusive and the user wants a decision, open or focus the job workspace with `uv run ptq open JOB_ID`; in that workspace have the agent load `@prime.md` and diagnose relatedness read-only from `PTQ_CONTEXT.md`, `worklog.md`, `report.md`, and the diff before suggesting code work. After triage, wait for explicit user approval before opening a fixer workspace, rerunning jobs, pushing commits, or cleaning anything.
